...
export default function DashboardPage() {
  ...
  const quickActions = [
    { name: 'Generate Chart', href: '/dashboard/charts/new', icon: BarChart3, color: 'violet' },
    { name: 'View Predictions', href: '/dashboard/predictions', icon: Zap, color: 'yellow' },
    { name: 'Book Consultation', href: '/dashboard/consultations', icon: Calendar, color: 'blue' },
    { name: 'Check Compatibility', href: '/dashboard/compatibility', icon: Users, color: 'pink' },
  ];
          ...
          <div className="flex items-start justify-between flex-wrap gap-6">
            <div className="flex-1 min-w-[300px]">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-white mb-3">
                Welcome back, <br />
                <span className="text-white/90">{user?.name || user?.email?.split('@')[0]}!</span>
              </h1>
              <p className="text-white/80 text-lg mb-8 max-w-2xl leading-relaxed">
                Your cosmic journey continues. Explore your charts, book consultations, or discover compatibility insights.
              </p>
              ...
              <div className="flex flex-wrap gap-3">
                {quickActions.map((action) => (
                  <Link
                    key={action.name}
                    href={action.href}
                    className="flex items-center gap-2 px-5 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-xl border border-white/20 rounded-xl text-white font-semibold transition-all hover:scale-105 hover:shadow-2xl"
                  >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => {
          ...
          return (
            <div
              key={index}
              className="group relative bg-white rounded-2xl p-6 border border-gray-200 hover:border-violet-500 hover:shadow-2xl transition-all duration-500 overflow-hidden hover:-translate-y-2"
            >
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className={`px-3 py-1 bg-gradient-to-r ${stat.gradient} rounded-full flex items-center gap-1.5 text-white text-xs font-bold shadow-lg`}>
