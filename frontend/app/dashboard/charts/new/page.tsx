  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profileResponse.ok) {
        ...
        alert(`Failed to create profile: ${errorData.detail || 'Unknown error'}`);
        return;
      }
      ...
      const chartResponse = await fetch('http://localhost:8000/api/v1/charts/natal', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (chartResponse.ok) {
        ...
        alert(`Failed to create chart: ${errorData.detail || 'Unknown error'}`);
      }
    } catch (error) {
