...
const SECURE_API_URL = API_URL?.startsWith('https') ? API_URL : 'https://192.168.0.102';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      if (oauthToken) {
        console.log('[DASHBOARD] OAuth token received:', oauthToken.substring(0, 20) + '...');
      ...
      console.log('[DASHBOARD v3.0] Token from localStorage:', token ? token.substring(0, 20) + '...' : 'NONE');
      console.log('[DASHBOARD v3.0] Current protocol:', typeof window !== 'undefined' ? window.location.protocol : 'N/A');
      console.log('[DASHBOARD v3.0] SECURE_API_URL:', SECURE_API_URL);
      
      if (!token) {
        console.log('[DASHBOARD v3.0] No token, redirecting to login');
        router.push('/auth/login');
        return;
      }

      try {
        const apiEndpoint = `${SECURE_API_URL}/api/v1/auth/me`;
        console.log('[DASHBOARD v3.0] Full endpoint:', apiEndpoint);
        console.log('[DASHBOARD v3.0] Token length:', token?.length);
        ...
        // Check if profile is complete (after user loads) - temporarily disabled for debugging
  ...
  const checkProfileComplete = async (token: string) => {
    try {
      ...
      if (response.ok) {
        ...
        if (profiles.length === 0 || !profiles[0].birth_date) {
          // No profile or incomplete profile - show modal
