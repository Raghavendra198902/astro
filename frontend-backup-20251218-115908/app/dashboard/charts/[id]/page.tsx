...
export default function ChartDetailPage() {
  ...
  useEffect(() => {
    if (activeTab === 'insights' && !interpretation && chart) {
      fetchInterpretation();
    }
  }, [activeTab, chart]);
  ...
  const fetchInterpretation = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/v1/interpretations/natal/${chartId}`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          style: 'detailed',
          focus_areas: ['personality', 'life_purpose', 'relationships', 'career']
        })
      });
  ...
  const fetchChartDetails = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/api/v1/charts/${chartId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950/30 to-slate-950 p-4 md:p-6 lg:p-8">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="flex gap-3 mb-8 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-purple-500/50 scrollbar-track-slate-800/50">
          {['overview', 'insights', 'planets', 'houses', 'aspects', 'yogas', 'dashas', 'shadbala'].map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'insights' ? '✨ Life Insights' : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
        ...
        <div className="bg-gradient-to-br from-slate-800/40 to-slate-900/40 backdrop-blur-xl rounded-2xl p-6 md:p-8 border border-slate-700/50 shadow-2xl">
          ...
          {activeTab === 'insights' && (
            <div>
              <h2 className="text-3xl font-bold bg-gradient-to-r from-white via-purple-200 to-pink-200 bg-clip-text text-transparent mb-8 flex items-center gap-3">
                <span className="text-4xl">✨</span>
                Life Insights - AI/ML Analysis
              </h2>
              
              {interpretationLoading ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-purple-500 mb-4"></div>
                  <p className="text-purple-400 text-lg">Generating your detailed AI insights...</p>
                  <p className="text-slate-500 text-sm mt-2">This may take a few moments</p>
                </div>
              ) : interpretation && interpretation.output_data?.ml_insights ? (
                <div className="space-y-6">
                  {/* Personality Profile */}
                  {interpretation.output_data.ml_insights.personality_profile && (
                    <div className="bg-gradient-to-br from-purple-600/20 via-purple-700/20 to-purple-800/20 rounded-2xl p-6 border border-purple-500/30 hover:border-purple-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-purple-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">🧠</span>
                        <h3 className="text-2xl font-bold text-white">Personality Profile</h3>
                      </div>
                      <p className="text-slate-200 leading-relaxed">{interpretation.output_data.ml_insights.personality_profile}</p>
                    </div>
                  )}

                  {/* Life Purpose */}
                  {interpretation.output_data.ml_insights.life_purpose_analysis && (
                    <div className="bg-gradient-to-br from-pink-600/20 via-pink-700/20 to-pink-800/20 rounded-2xl p-6 border border-pink-500/30 hover:border-pink-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-pink-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">🎯</span>
                        <h3 className="text-2xl font-bold text-white">Life Purpose & Mission</h3>
                      </div>
                      <p className="text-slate-200 leading-relaxed">{interpretation.output_data.ml_insights.life_purpose_analysis}</p>
                    </div>
                  )}

                  {/* Relationship Compatibility */}
                  {interpretation.output_data.ml_insights.relationship_compatibility && (
                    <div className="bg-gradient-to-br from-rose-600/20 via-rose-700/20 to-rose-800/20 rounded-2xl p-6 border border-rose-500/30 hover:border-rose-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-rose-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">💕</span>
                        <h3 className="text-2xl font-bold text-white">Relationship Patterns</h3>
                      </div>
                      <p className="text-slate-200 leading-relaxed">{interpretation.output_data.ml_insights.relationship_compatibility}</p>
                    </div>
                  )}

                  {/* Career Potential */}
                  {interpretation.output_data.ml_insights.career_potential && (
                    <div className="bg-gradient-to-br from-blue-600/20 via-blue-700/20 to-blue-800/20 rounded-2xl p-6 border border-blue-500/30 hover:border-blue-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">💼</span>
                        <h3 className="text-2xl font-bold text-white">Career Talents & Potential</h3>
                      </div>
                      <p className="text-slate-200 leading-relaxed">{interpretation.output_data.ml_insights.career_potential}</p>
                    </div>
                  )}

                  {/* Strengths */}
                  {interpretation.output_data.ml_insights.strength_areas && interpretation.output_data.ml_insights.strength_areas.length > 0 && (
                    <div className="bg-gradient-to-br from-green-600/20 via-green-700/20 to-green-800/20 rounded-2xl p-6 border border-green-500/30 hover:border-green-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-green-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">💪</span>
                        <h3 className="text-2xl font-bold text-white">Your Strengths</h3>
                      </div>
                      <ul className="space-y-2">
                        {interpretation.output_data.ml_insights.strength_areas.map((strength: string, i: number) => (
                  ...
                  {interpretation.output_data.ml_insights.growth_opportunities && interpretation.output_data.ml_insights.growth_opportunities.length > 0 && (
                    <div className="bg-gradient-to-br from-amber-600/20 via-amber-700/20 to-amber-800/20 rounded-2xl p-6 border border-amber-500/30 hover:border-amber-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">🌱</span>
                        <h3 className="text-2xl font-bold text-white">Growth Opportunities</h3>
                      </div>
                      <ul className="space-y-2">
                        {interpretation.output_data.ml_insights.growth_opportunities.map((growth: string, i: number) => (
                          <li key={i} className="flex items-start gap-2 text-slate-200">
                  ...
                  {/* Lucky Periods */}
                  {interpretation.output_data.ml_insights.lucky_periods && interpretation.output_data.ml_insights.lucky_periods.length > 0 && (
                    <div className="bg-gradient-to-br from-yellow-600/20 via-yellow-700/20 to-yellow-800/20 rounded-2xl p-6 border border-yellow-500/30 hover:border-yellow-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-yellow-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">🍀</span>
                        <h3 className="text-2xl font-bold text-white">Favorable Periods</h3>
                      </div>
                      <ul className="space-y-2">
                        {interpretation.output_data.ml_insights.lucky_periods.map((period: string, i: number) => (
                          <li key={i} className="flex items-start gap-2 text-slate-200">
                  ...
                  {/* Challenges */}
                  {interpretation.output_data.ml_insights.challenge_areas && interpretation.output_data.ml_insights.challenge_areas.length > 0 && (
                    <div className="bg-gradient-to-br from-red-600/20 via-red-700/20 to-red-800/20 rounded-2xl p-6 border border-red-500/30 hover:border-red-400/50 transition-all duration-300 hover:shadow-xl hover:shadow-red-500/20">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">⚡</span>
                        <h3 className="text-2xl font-bold text-white">Challenge Areas</h3>
                      </div>
                      <ul className="space-y-2">
                        {interpretation.output_data.ml_insights.challenge_areas.map((challenge: string, i: number) => (
                          <li key={i} className="flex items-start gap-2 text-slate-200">
                  ...
                  {interpretation.interpretation_text && (
                    <div className="bg-gradient-to-br from-slate-700/40 to-slate-800/40 backdrop-blur-sm rounded-2xl p-6 border border-slate-600/50 mt-8">
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-3xl">📜</span>
                        <h3 className="text-2xl font-bold text-white">Complete AI Interpretation</h3>
                      </div>
                      <div className="text-slate-200 leading-relaxed whitespace-pre-wrap">{interpretation.interpretation_text}</div>
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center py-16">
                  ...
                  <p className="text-slate-400 text-lg mb-4">No insights available yet</p>
                  <button
                    onClick={fetchInterpretation}
                    className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 rounded-xl transition-all duration-300 text-white font-semibold shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 hover:scale-105"
                  >
